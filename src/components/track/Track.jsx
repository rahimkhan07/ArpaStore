  ...
  return (
    ...
    >
      <div className="container px-5 md:py-5 mx-auto">
        <div className="flex flex-wrap m-0 sm:-m-4  text-center">
          {features.map((item, index) => (
            <div key={index} className="p-4 md:w-1/3 sm:w-1/2 w-full">
              <div
                className="border rounded-lg px-4 py-6 hover:shadow-md transition-all duration-300 ease-in-out"
                style={{
                  borderColor: "#ddd",
              ...
              >
                <svg
                  ...
                  strokeWidth={1.5}
                  stroke="#333333ff"
                  className="w-15 h-15 mb-3 inline-block"
                >
                  {item.icon}
                </svg>
                <h2
                  className="title-font font-semibold text-lg mb-2"
                  style={{ color: mode === "dark" ? "#F3D0D7" : "#F3D0D7" }}
                >
                  {item.title}
                </h2>
                <p
                  className="leading-relaxed text-sm"
                  style={{ color: mode === "dark" ? "#ddd" : "#666" }}
                >
                  {item.desc}
                </p>
              </div>
